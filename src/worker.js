...
function financialRequestPrintableHtml(record, type) {
  ...
  return `<section class="payment-request-print"><header><strong>MEDLANE DIAGNOSTIC SOLUTIONS, INC.</strong><span>${escapeHtml(record.id)}</span></header><div class="pr-meta"><span>${partyLabel}: <strong>${escapeHtml(partyValue || "-")}</strong></span>${extraMeta}<span>Status: <strong>${escapeHtml(record.requestStatus || record.status || "-")}</strong></span></div><table><thead><tr><th>Particulars</th><th>Amount</th></tr></thead><tbody>${items.map((item) => `<tr><td>${escapeHtml(item.particulars || "")}</td><td>${money(item.amount || 0)}</td></tr>`).join("")}<tr><td><strong>Total</strong></td><td><strong>${money(record.amount || 0)}</strong></td></tr></tbody></table>${record.requestNote ? `<p class="pr-instructions"><strong>Notes:</strong> ${escapeHtml(record.requestNote)}</p>` : ""}<footer><div>Payment Method:<br><strong>${escapeHtml(record.method || "Not set")}</strong></div><div>Approved by:<br><strong>${escapeHtml(record.approvedBy || "Pending")}</strong></div></footer></section>`;
}
...
function productIssueParameterRows(report) {
  const rows = report.qcParameters?.length ? report.qcParameters : [];
  if (!rows.length) return `<tr><td colspan="14">No parameters recorded.</td></tr>`;
  return rows.map((row) => `<tr><td>${escapeHtml(row.parameter || "-")}</td><td>${escapeHtml(row.factor || "-")}</td><td>${escapeHtml(row.l1Range || "-")}</td><td>${escapeHtml(row.l1Result || "-")}</td><td>${row.l1P ? "✓" : ""}</td><td>${row.l1F ? "✓" : ""}</td><td>${escapeHtml(row.l2Range || "-")}</td><td>${escapeHtml(row.l2Result || "-")}</td><td>${row.l2P ? "✓" : ""}</td><td>${row.l2F ? "✓" : ""}</td><td>${escapeHtml(row.l3Range || "-")}</td><td>${escapeHtml(row.l3Result || "-")}</td><td>${row.l3P ? "✓" : ""}</td><td>${row.l3F ? "✓" : ""}</td></tr>`).join("");
}

function productIssueParameterTableHtml(report) {
  return `<table class="qc-parameter-print-table"><thead><tr><th rowspan="3">Parameters</th><th rowspan="3">Calibrator Lot:<br>Factor</th><th colspan="12">Control</th></tr><tr><th colspan="4">Level 1<br>Lot: ${escapeHtml(report.qcLevel1Lot || "-")}</th><th colspan="4">Level 2<br>Lot: ${escapeHtml(report.qcLevel2Lot || "-")}</th><th colspan="4">Level 3<br>Lot: ${escapeHtml(report.qcLevel3Lot || "-")}</th></tr><tr><th>Reference Range</th><th>Result</th><th>P</th><th>F</th><th>Reference Range</th><th>Result</th><th>P</th><th>F</th><th>Reference Range</th><th>Result</th><th>P</th><th>F</th></tr></thead><tbody>${productIssueParameterRows(report)}</tbody></table>`;
}
