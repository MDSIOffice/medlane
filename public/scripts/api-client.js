const MedlaneAPI = (() => {
  ...
  async function runBirthdayGreetingTest() {
    return request("/api/discord/birthday-test", { method: "POST" });
  }
  ...
  async function listGameLeaderboard(by = "score") {
    return request(`/api/game/leaderboard?by=${encodeURIComponent(by)}`);
  }
