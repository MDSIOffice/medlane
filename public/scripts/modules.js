...
function lineChart(entries) {
  ...
  return `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Monthly sales line graph">
    <defs><linearGradient id="salesTrendGradient" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stop-color="#3db3cd" stop-opacity="0.28"/><stop offset="1" stop-color="#3db3cd" stop-opacity="0"/></linearGradient></defs>
...
function multiSeriesChart(labels, series, options = {}) {
  ...
  const defs = gradSeries.length
    ? `<defs>${gradSeries.map((s) => {
        const id = `msc-bar-${barSeries.indexOf(s)}`;
        return `<linearGradient id="${id}" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="${s.gradient[0]}"/><stop offset="1" stop-color="${s.gradient[1]}"/></linearGradient>`;
      }).join("")}</defs>`
    : "";
  const tip = (s, index) => `${escapeHtml(s.label)} — ${escapeHtml(String(labels[index]))}: ${s.format ? s.format(s.values[index] || 0) : s.values[index] || 0}`;
  const bars = barSeries.map((s, si) => labels.map((_, index) => {
...
function openPaymentRequestMoreMenu(cvNo) {
  qs("#payment-request-more-modal")?.remove();
...
function showSection(sectionId, options = {}) {
  ...
  if (options.render !== false && appInitialRenderComplete) renderSection(sectionId);
...
function populateSalesClientFilter(sales) {
  const list = qs("#sales-client-options");
  if (!list) return;
  const clients = [...new Set(sales.map((s) => s.client).filter(Boolean))].sort((a, b) => String(a).localeCompare(String(b)));
  list.innerHTML = clients.map((name) => `<option value="${escapeHtml(name)}"></option>`).join("");
}
...
function renderSales() {
  ...
  populateSalesClientFilter(scoped);
  const clientQuery = String(qs("#sales-client")?.value || "").trim().toLowerCase();
  const rows = scoped.filter((s) => !clientQuery || String(s.client || "").toLowerCase().includes(clientQuery)).filter((s) => status === "all" || statusForSale(s) === status).filter((s) => type === "all" || documentType(s.type) === type).filter((s) => includesSearch(Object.values(s))).reverse();
...
function attachedFilesHtml(recordType, recordId) {
  ...
  return `<div class="attached-files-list">${files.map((file) => `<button class="attached-file-chip" type="button" data-view-file="${escapeHtml(file.id)}">📎 ${escapeHtml(file.file_name)}</button>`).join("")}</div>`;
}
...
function renderInvoicing() {
  ...
  if (invoiceViewMode === "table") {
    table("#invoice-table", ["Document", "Client", "Area", "Date", "Total Due", "Paid", "Balance", "Status", "Delivery", "Actions"], rows.map((s) => {
      const paid = Number(s.paid || 0);
      const balance = Math.max(Number(s.net || 0) - paid, 0);
      return { focus: s.documentNo || s.id, cells: [`<span class="invoice-type-badge type-${escapeHtml(documentType(s.type))}"><span>${invoiceTypeIcon(s.type)}</span>${escapeHtml(s.documentNo || s.id)}</span>`, s.client, s.area || "-", s.date, peso.format(s.net), peso.format(paid), peso.format(balance), `<span class="pill ${statusClass(statusForSale(s))}">${invoiceTypeLabel(s.type)} · ${statusForSale(s)}</span>`, deliveryControl(s), actionCell(s, true)] };
    }));
  }
  qs("#invoice-grid").innerHTML = rows.map((s) => {
    const due = fmtDate(addDays(s.date, s.terms));
    const paid = Number(s.paid || 0);
    const pending = Math.max(Number(s.net || 0) - paid, 0);
    const balanceLine = paid > 0 ? `<small class="invoice-balance-line">Paid ${peso.format(paid)} · Pending ${peso.format(pending)}</small>` : "";
...
const printableCompanyFooterHtml = "MEDLANE DIAGNOSTIC SOLUTIONS, INC.<br>13 Gumamela St. Pilar Village, Las Piñas<br>Las Piñas City 1740<br>Tel. No. (02) 8836-2853 Email: receivables.mdsi@gmail.com";
function printableBrandHeaderHtml(title = "") {
...
function printableFooterHtml() { return `<div class="print-company-footer">${printableCompanyFooterHtml}</div>`; }
...
function syncFinancialRequestTotal() {
  ...
  if (preview) preview.innerHTML = modalType === "payable" ? `<div class="preview-tax-label">Optional withholding deductions computed from VAT-exclusive base</div><div class="invoice-tax-summary live-preview"><div class="invoice-meta"><span>Gross Payable</span><strong>${peso.format(gross)}</strong></div><div class="invoice-meta"><span>VAT-exclusive Base</span><strong>${withholdingMoney(taxBase)}</strong></div><div class="invoice-meta"><span>Withholding 1%</span><strong>${withholdingMoney(withholdingTax1)}</strong></div><div class="invoice-meta"><span>Withholding 2%</span><strong>${withholdingMoney(withholdingTax2)}</strong></div><div class="invoice-meta total-line"><span>Net Payable</span><strong>${peso.format(total)}</strong></div></div>` : "";
}

function roundMoney(value) { return Math.round(Number(value || 0) * 100) / 100; }
function withholdingMoney(value) { return Number(value || 0).toLocaleString("en-PH", { style: "currency", currency: "PHP", minimumFractionDigits: 2, maximumFractionDigits: 2 }); }
...
function togglePaymentRequestChequeFields() {
  ...
  syncPaymentRequestBankAccount();
  ["bank", "bankAccount", "cheque", "chequeDate", "transferDate"].forEach((id) => {
    const field = qs(`#${id}`)?.closest(".field");
    if (!field) return;
    const visible = id === "bank" ? ["Check", "Bank Transfer"].includes(type) : id === "bankAccount" || id === "transferDate" ? type === "Bank Transfer" : type === "Check";
    field.hidden = !visible;
    qs(`#${id}`).required = (type === "Check" && id === "bank") || (type === "Bank Transfer" && ["bank", "transferDate"].includes(id));
    if (!visible && id !== "bankAccount") qs(`#${id}`).value = "";
  });
}

function syncPaymentRequestBankAccount() {
  const bank = data.banks.find((entry) => entry.name === qs("#bank")?.value);
  if (qs("#bankAccount")) qs("#bankAccount").value = bank?.account || "";
}
...
async function renderLeafletCollectionMap() {
  ...
  if (!mapEl) return;
  await loadLeafletAssets().catch(() => null);
  if (!window.L) {
...
function soaHtml(client) {
  ...
  const rows = invoices.map((sale) => {
    const invoiceDate = new Date(`${sale.date}T00:00:00`);
    const soaDate = new Date(`${fmtDate(today)}T00:00:00`);
    const daysDue = Number.isNaN(invoiceDate.getTime()) ? 0 : Math.max(0, Math.floor((soaDate.getTime() - invoiceDate.getTime()) / 86400000));
    const balance = Math.max(Number(sale.net || 0) - Number(sale.paid || 0), 0);
...
function financialRequestDetailFields(record, type) {
  ...
  if (type === "payable") fields.push(["Gross", peso.format(record.grossAmount || record.amount || 0)], ["Withholding 1%", withholdingMoney(record.withholdingTax1 || 0)], ["Withholding 2%", withholdingMoney(record.withholdingTax2 || 0)], ["Net Amount", peso.format(record.amount || 0)]);
...
let appInitialRenderComplete = false;
const sectionRenderers = {
  dashboard: () => { renderDashboard(); renderWorkflowAssist("dashboard"); },
  ...
  "receivables-tracker": renderReceivablesTracker,
  "client-invoices": renderClientInvoices,
  warranty: renderWarranty,
  "product-issues": renderProductIssues,
  "purchase-history": renderPurchaseHistory,
  imports: renderImports,
  payables: renderPayables,
  replenishments: renderReplenishments,
...
function renderSection(sectionId = document.body.dataset.activeSection || "dashboard") {
  ...
  renderUserMenu();
  if (sectionId !== "user-settings") renderUserSettings();
}
function renderAll() {
...
function openModal(type, edit = null) {
  ...
  qs("#modal-fields").innerHTML = fields.map(([name, label, kind = "text", options]) => {
    ...
    if (kind === "datalist" || kind === "datalist-optional") {
      const values = typeof options === "function" ? options() : options;
      return `<div class="field${full}"><label for="${name}">${label}</label><input id="${name}" name="${name}" list="${name}-options" autocomplete="off" ${kind === "datalist" ? "required" : ""} /><datalist id="${name}-options">${values.map((value) => `<option value="${escapeHtml(value)}"></option>`).join("")}</datalist></div>`;
    }
    ...
    return `<div class="field${full}"><label for="${name}">${label}</label><input id="${name}" name="${name}" type="${kind}" ${kind === "date" && name.toLowerCase().includes("expiry") ? `min="${fmtDate(today)}"` : ""} ${kind === "date" && name === "birthday" ? `max="${latestAllowedBirthday()}"` : ""} ${kind === "number" && name === "discount" ? `step="any"` : ""} ${(kind !== "date" && name !== "creditLimit") || name === "birthday" ? "required" : ""} /></div>`;
  }).join("");
